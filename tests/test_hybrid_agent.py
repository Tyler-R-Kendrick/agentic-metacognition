...
def test_steered_executor_collects_trace_and_runs_selected_controller(
    ...
    assert 1 <= len(result.activation_trace.top_feature_scores) <= 3
    assert {
...
def test_hybrid_meta_cognition_agent_falls_back_and_records_memory():
    controller = steering.SteeringController(
        controller_id="retrieve_before_commit_v1",
    ...
    assert run.selected_controller_id == "retrieve_before_commit_v1"
    ...
    assert executor.calls == ["retrieve_before_commit_v1", None]
